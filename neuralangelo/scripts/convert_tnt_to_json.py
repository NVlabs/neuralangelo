...
from neuralangelo.scripts.convert_data_to_json import export_to_json  # NOQA
...
def create_init_files(pinhole_dict_file, db_file, out_dir):
    ...
    for img_name in pinhole_dict:
        ...
        img_line = images_line_template.format(
            image_id="{image_id}",
            qw=qvec[0],
            qx=qvec[1],
            qy=qvec[2],
...
def init_colmap(args):
    ...
    for scene in scene_list:
        ...
        if not os.path.exists(f"{scene_path}/images_raw"):
            raise Exception(
                f"'images_raw` folder cannot be found in {scene_path}."
                "Please check the expected folder structure in DATA_PREPROCESSING.md"
            )

        # extract features
        os.system(
            f"colmap feature_extractor --database_path {scene_path}/database.db \
        ...
        # read poses
        poses = load_COLMAP_poses(
            os.path.join(scene_path, f"{scene}_COLMAP_SfM.log"),
        ...
        cameras, images, points3D = read_model(
            os.path.join(args.tnt_path, scene, "sparse"), ext=".bin"
        )
        export_to_json(
            cameras,
            images,
        ...
        print(
            "Writing data to json file: ", os.path.join(scene_path, "transforms.json")
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--tnt_path", type=str, default=None, help="Path to tanks and temples dataset"
    )
